...
VRAM = 0x80010000
DISC_EXE = os.path.join(REPO, "disc", "SLUS_014.11")

_payload = None


def payload():
    """The original executable, minus its 0x800-byte PS-X EXE header.

    Read on first use rather than at import.  build.py and verify_src.py both
    import this module, and `disc/` is gitignored -- so reading it eagerly meant
    a machine that had not extracted the disc yet died with a bare
    FileNotFoundError traceback from an import line, naming neither the file nor
    the command that produces it.
    """
    global _payload
    if _payload is None:
        if not os.path.exists(DISC_EXE):
            sys.exit("%s missing -- run tools/extract_disc.py first "
                     "(it is gitignored; you supply your own disc)" % DISC_EXE)
        _payload = open(DISC_EXE, "rb").read()[0x800:]
    return _payload
...
def asm_inventory():
    ...
    # Same reasoning as payload(): asm/ is generated by splat and gitignored, so
    # on a fresh checkout this is the first thing to be missing and the bare
    # listdir traceback did not say what to run.
    if not os.path.isdir(asm_dir):
        sys.exit("%s missing -- run tools/all.py (or the split step) first"
                 % asm_dir)
