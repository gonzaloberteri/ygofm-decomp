...
# NOTE the leading whitespace: these lines are indented ("  .L800708B0:"), and an
# `^\.L`-anchored pattern silently matched none of them -- which proposed splits
# in the middle of real functions.  Worse, once a bad symbol exists splat renders
# that address as `func_X` rather than `.L X`, so the mistake becomes
# self-justifying on the next run.
LOCAL_LABEL = re.compile(r"^\s*\.L[0-9A-Fa-f]{8}:")
...
def main():
    ...
    if args.write and props:
        # The output must ACCUMULATE, not be rewritten.  This tool reads `asm/`,
        # which already reflects the splits applied on previous runs -- so a
        # freshly computed set only contains what is *still* merged, and writing
        # it verbatim erases every split that made the current disassembly what
        # it is.  Doing exactly that dropped 320 earlier splits and left 104
        # source files referring to functions that no longer existed.
        #
        # Because each round exposes spans the previous round's labels revealed,
        # re-run until the count stops growing.
        existing = {}
        if os.path.exists(SYMBOLS):
            for line in open(SYMBOLS):
                m = re.match(r"(\w+)\s*=\s*0x([0-9A-Fa-f]+)\s*;", line.strip())
                if m:
                    existing[int(m.group(2), 16)] = m.group(1)

        before = len(existing)
        for p in props:
            existing.setdefault(p["addr"], "func_%08X" % p["addr"])
        ...
        with open(SYMBOLS, "w") as fp:
            fp.write("// Generated by tools/split_funcs.py -- do not edit.\n"
                     ...
                     "// Accumulated across runs; re-run until the count settles.\n")
            for addr in sorted(existing):
                fp.write("%s = 0x%08X; // type:func\n" % (existing[addr], addr))
        added = len(existing) - before
        print("%d symbol(s) total in %s (%d new this run)"
              % (len(existing), SYMBOLS, added))
